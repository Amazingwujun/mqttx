...
public final class ConnectHandler extends AbstractMqttTopicSecureHandler {
        ...
        if (isCleanSession) {
            actionOnCleanSession(clientId)
                    .doOnSuccess(unused -> {
                        ...
                        onClientConnectSuccess(ctx)
                                .doOnSuccess(v -> {
                                    if (!channel.isActive()) {
                                        log.warn("会话过期: clientId:[{}] channel:[{}]", clientId, channel.id().asShortText());
                                        return;
                                    }
            ...
            sessionService.find(clientId)
                    ...
                    .doOnSuccess(s -> {
                        ...
                        republish(ctx).doOnError(t -> log.error(t.getMessage(), t))
                                .subscribe();
                    })
                    .doOnError(t -> log.error(t.getMessage(), t))
                    .subscribe();
    ...
    private Mono<Void> onClientConnectSuccess(ChannelHandlerContext ctx) {
        if (enableSysTopic) {
            ...
            var connectTime = Unpooled.buffer(bytes.length).writeBytes(bytes);
